...
const MCTP_PHYS_BINDING_UNSPEC: u8 = 0x00;
...
impl Nla for AfSpecMctp {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        use self::AfSpecMctp::*;
        match *self {
            Net(ref value) => emit_u32(buffer, *value).unwrap(),
            PhysBinding(ref b) => buffer[0] = b.into(),
    ...
    fn kind(&self) -> u16 {
        ...
        match *self {
            Net(_) => IFLA_MCTP_NET,
            PhysBinding(_) => IFLA_MCTP_PHYS_BINDING,
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for AfSpecMctp {
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            IFLA_MCTP_NET => {
                Net(parse_u32(payload)
                    .context("invalid IFLA_MCTP_NET value")?)
            }
            IFLA_MCTP_PHYS_BINDING => {
                let b = parse_u8(payload)
                    .context("invalid IFLA_MCTP_PHYS_BINDING value")?;
                PhysBinding(MctpPhysBinding::from(b))
            }
...
impl From<u8> for MctpPhysBinding {
    fn from(d: u8) -> Self {
        match d {
            MCTP_PHYS_BINDING_UNSPEC => Self::Unspec,
...
impl From<&MctpPhysBinding> for u8 {
    fn from(v: &MctpPhysBinding) -> Self {
        match v {
            MctpPhysBinding::Unspec => MCTP_PHYS_BINDING_UNSPEC,
