    ...
    for RouteSeg6LocalIpTunnel
{
    fn parse(
        ...
        Ok(match buf.kind() {
            ...
            SEG6_LOCAL_SRH => Self::SRH(SegmentRoutingHeader::parse(buf)?),
...
impl Nla for SegmentRoutingHeader {
    fn value_len(&self) -> usize {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        ...
        SegmentRoutingHeader::push_segments(
            seg6_header.segments_mut(),
            segments,
        );
    }
}
    ...
    for SegmentRoutingHeader
{
    fn parse(
        ...
        SegmentRoutingHeader::get_segments(
        ...
        Ok(SegmentRoutingHeader { segments })
    }
}
